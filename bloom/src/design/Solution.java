 ...
 // TC: O(1)
    public int get(int key) {
        if(!record.containsKey(key))
            return -1;
//Since we treat "get()" as an event (as it helps us understand how recently a 'key' was accessed), 
//we need a 'remove-add' operation on "record" to record the key's order of access.
...
key needs to be discarded as it gets outdated. 
The new "key,value" is required to be added a new entry in "record".**/
        if(record.containsKey(key))
            record.remove(key);
